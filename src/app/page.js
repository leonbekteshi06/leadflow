...
export default function CRM() {
  ...
  const markSent = async (id) => { const c = contacts.find(x => x.id === id); if (!c) return; const ns = (c.current_step || 0) + 1; const seq = c.outreach_sequence || "Default"; let seqMsgs = messages.filter(m => (m.sequence_name || "Default") === seq).sort((a, b) => a.step - b.step); if (seqMsgs.length === 0) seqMsgs = messages.filter(m => (m.sequence_name || "Default") === "Default").sort((a, b) => a.step - b.step); const cm = seqMsgs[c.current_step || 0]; const nm = seqMsgs[ns]; const pv = pendingVariants[id]; const variant = pv?.label || "A"; const isFirst = (c.current_step || 0) === 0; const upd = { current_step: ns, stage: ns > 0 && ["new", "old"].includes(c.stage) ? "outreach" : c.stage, last_contacted_at: todayStr(), next_follow_up: nm ? addDays(todayStr(), nm.delay_days) : null, history: [...(c.history || []), { step: ns, name: cm?.name || `Msg ${ns}`, variant, at: todayStr() }] }; if (isFirst || !c.assigned_to) upd.assigned_to = user; await updateContact(id, upd); setPendingVariants(p => { const n = { ...p }; delete n[id]; return n; }); if (isFirst) { await logKpi(user, "DMs", 1); } logActivity(user, "sent_outreach", `${cm?.name} (v${variant}) to ${c.name}`); flash(`Marked sent! (Version ${variant})`); };
      ...
      <div>
        ...
        <div style={{ background: "#0F172A", borderRadius: 10, border: "1px solid #1E293B", maxHeight: 300, overflow: "auto" }}>
          {activityLog.length === 0 ? <div style={{ padding: 20, textAlign: "center", color: "#475569", fontSize: 12 }}>No activity yet</div>
            : activityLog.map(a => {
              const icons = { added_lead: "➕", sent_outreach: "📤", sent_nurture: "🔁", sent_loom: "🎥", pinged_lead: "📌", moved_stage: "📋", closed_deal: "🎉", logged_kpi: "📊", imported_csv: "📥", found_leads: "🔍" };
