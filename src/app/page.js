...
const needsAction = (c) => { if (c.stage === "responded" && c.loom_pending) return true; if (["new", "outreach", "old"].includes(c.stage)) { return c.next_follow_up ? daysDiff(todayStr(), c.next_follow_up) <= 0 : false; } if (c.stage === "responded") { return c.next_nurture_date ? daysDiff(todayStr(), c.next_nurture_date) <= 0 : false; } return false; };
const urgency = (c) => { if (c.stage === "responded" && c.loom_pending) return -1; if (["new", "outreach", "old"].includes(c.stage)) { return c.next_follow_up ? daysDiff(todayStr(), c.next_follow_up) : 999; } if (c.stage === "responded") { return c.next_nurture_date ? daysDiff(todayStr(), c.next_nurture_date) : 999; } return 999; };
...
export default function CRM() {
  ...
  const Row = ({ c, showWho }) => {
    ...
    const isO = ["new", "outreach", "old"].includes(c.stage); const isLoom = c.stage === "responded" && c.loom_pending; const isN = c.stage === "responded" && !c.loom_pending && c.next_nurture_date;
      ...
      <div>
        ...
        <div style={{ background: "#0F172A", borderRadius: 10, border: "1px solid #1E293B", maxHeight: 300, overflow: "auto" }}>
          {activityLog.length === 0 ? <div style={{ padding: 20, textAlign: "center", color: "#475569", fontSize: 12 }}>No activity yet</div>
            : activityLog.map(a => {
              const icons = { added_lead: "➕", sent_outreach: "📤", sent_nurture: "🔁", sent_loom: "🎥", moved_stage: "📋", closed_deal: "🎉", logged_kpi: "📊", imported_csv: "📥", found_leads: "🔍" };
