...
export default function CRM() {
  ...
  const confirmLoom = async (id) => { const c = contacts.find(x => x.id === id); if (!c) return; await updateContact(id, { loom_pending: false, next_follow_up: null, next_nurture_date: todayStr(), last_contacted_at: todayStr() }); await logKpi(user, "Looms", 1); logActivity(user, "sent_loom", c.name); flash("Loom sent! Nurture sequence started."); };
