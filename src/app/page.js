...
const getSendDate = (c) => { if(!c.next_follow_up) return { text:"-", color:"#475569", bg:"transparent" }; if(c.current_step===0) return { text:"ASAP", color:"#F59E0B", bg:"#F59E0B18" }; const d = daysDiff(todayStr(), c.next_follow_up); if(d<0) return { text:`${Math.abs(d)}d overdue`, color:"#EF4444", bg:"#EF444418" }; if(d===0) return { text:"Today", color:"#F59E0B", bg:"#F59E0B18" }; if(d===1) return { text:"Tomorrow", color:"#3B82F6", bg:"#3B82F618" }; return { text:fmtEU(c.next_follow_up), color:"#94A3B8", bg:"#1E293B" }; };
const getNurtureDate = (c) => { if(!c.next_nurture_date) return null; const d = daysDiff(todayStr(), c.next_nurture_date); if(d<0) return { text:`${Math.abs(d)}d overdue`, color:"#EF4444", bg:"#EF444418" }; if(d===0) return { text:"Today", color:"#F59E0B", bg:"#F59E0B18" }; if(d===1) return { text:"Tomorrow", color:"#8B5CF6", bg:"#8B5CF618" }; return { text:fmtEU(c.next_nurture_date), color:"#94A3B8", bg:"#1E293B" }; };
...
export default function CRM() {
  ...
  const Detail = () => { const c = contacts.find(x => x.id === detailId); if (!c) return null; const nm = getNext(c); const nn = getNextN(c); return (<div style={S.detail}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}><div><h3 style={{ color: "#F1F5F9", fontSize: 17, fontWeight: 600, margin: 0 }}>{c.name}</h3><div style={{ color: "#64748B", fontSize: 11, marginTop: 3 }}>Added {fmtEU(c.created_at)} · Assigned to <strong style={{ color: "#CBD5E1" }}>{c.assigned_to}</strong></div></div><button style={S.x} onClick={() => setDetailId(null)}>✕</button></div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 12 }}>{c.ig && <div style={S.df}><span style={S.dl}>Instagram</span><span style={S.dv}>{c.ig}</span></div>}{c.email && <div style={S.df}><span style={S.dl}>Email</span><span style={S.dv}>{c.email}</span></div>}{c.youtube && <div style={S.df}><span style={S.dl}>YouTube</span><span style={S.dv}>{c.youtube}</span></div>}{c.website && <div style={S.df}><span style={S.dl}>Website</span><span style={S.dv}>{c.website}</span></div>}{c.pipeline_value > 0 && <div style={S.df}><span style={S.dl}>Pipeline Value</span><span style={{ ...S.dv, color: "#10B981" }}>{fmtMoney(c.pipeline_value)}</span></div>}{c.closed_value > 0 && <div style={S.df}><span style={S.dl}>Closed For</span><span style={{ ...S.dv, color: "#10B981" }}>{fmtMoney(c.closed_value)}</span></div>}</div>{c.notes && <div style={{ ...S.df, marginTop: 8 }}><span style={S.dl}>Notes</span><span style={{ ...S.dv, whiteSpace: "pre-wrap" }}>{c.notes}</span></div>}{nm && ["new", "outreach"].includes(c.stage) && (<div style={{ marginTop: 12, padding: 10, background: "#0B1120", borderRadius: 8, border: "1px solid #1E293B" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}><span style={{ color: "#94A3B8", fontSize: 10, fontWeight: 700, textTransform: "uppercase" }}>NEXT: {nm.name}</span><button style={S.sc} onClick={() => copy(c, "outreach")}>{copied === c.id + "outreach" ? "Copied!" : "Copy"}</button></div><div style={{ color: "#CBD5E1", fontSize: 12, lineHeight: 1.5, whiteSpace: "pre-wrap" }}>{nm.body}</div></div>)}{nn && c.stage === "responded" && (<div style={{ marginTop: 12, padding: 10, background: "#0B1120", borderRadius: 8, border: "1px solid #8B5CF620" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}><span style={{ color: "#C4B5FD", fontSize: 10, fontWeight: 700, textTransform: "uppercase" }}>NURTURE: {nn.name}</span><button style={{ ...S.sc, borderColor: "#8B5CF640", color: "#C4B5FD" }} onClick={() => copy(c, "nurture")}>{copied === c.id + "nurture" ? "Copied!" : "Copy"}</button></div><div style={{ color: "#CBD5E1", fontSize: 12, lineHeight: 1.5, whiteSpace: "pre-wrap" }}>{nn.body}</div></div>)}{((c.history || []).length > 0 || (c.nurture_history || []).length > 0) && (<div style={{ marginTop: 12 }}><span style={{ color: "#64748B", fontSize: 10, fontWeight: 700, textTransform: "uppercase" }}>Activity</span><div style={{ marginTop: 4, display: "flex", flexDirection: "column", gap: 3 }}>{[...(c.history || []).map(h => ({ ...h, t: "out" })), ...(c.nurture_history || []).map(h => ({ ...h, t: "nur" }))].sort((a, b) => b.at > a.at ? 1 : -1).map((h, i) => (<div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid #1E293B" }}><span style={{ color: h.t === "nur" ? "#C4B5FD" : "#CBD5E1", fontSize: 11 }}>{h.t === "nur" ? "🔁 " : "📤 "}{h.name}</span><span style={{ color: "#64748B", fontSize: 10 }}>{fmtEU(h.at)}</span></div>))}</div></div>)}</div>); };

  const MsgView = ({ type }) => { const list = type === "outreach" ? messages : nurtureM; return (<div style={S.content}><div style={S.header}><div><h1 style={S.h1}>{type === "outreach" ? "Outreach Sequence" : "Nurture Sequence"}</h1><p style={S.sub}>{type === "outreach" ? "Edit to match your offer." : "Follow-up messages for responded leads."}</p></div><button style={S.pri} onClick={() => setModal({ type: "msg", data: null, msgType: type })}>+ Add</button></div><div style={{ display: "flex", flexDirection: "column" }}>{list.sort((a, b) => a.step - b.step).map((m, i) => (<div key={m.id}><div style={{ background: "#0F172A", borderRadius: 10, border: `1px solid ${type === "nurture" ? "#8B5CF620" : "#1E293B"}`, overflow: "hidden" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", borderBottom: "1px solid #1E293B" }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ width: 24, height: 24, borderRadius: "50%", background: type === "nurture" ? "linear-gradient(135deg,#8B5CF6,#EC4899)" : "linear-gradient(135deg,#3B82F6,#6366F1)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700 }}>{m.step}</div><div><div style={{ color: "#F1F5F9", fontWeight: 600, fontSize: 13 }}>{m.name}</div><div style={{ color: "#64748B", fontSize: 10 }}>{m.channel === "ig" ? "📱 IG" : "📧 Email"} · {m.delay_days === 0 ? "Immediately" : `${m.delay_days}d`}</div></div></div><div style={{ display: "flex", gap: 3 }}><button style={{ ...S.act, color: "#94A3B8" }} onClick={() => setModal({ type: "msg", data: m, msgType: type })}>✎</button>{list.length > 1 && <button style={{ ...S.act, color: "#EF4444" }} onClick={() => deleteMsg(m.id, type)}>✕</button>}</div></div><div style={{ padding: "10px 12px", fontSize: 12, lineHeight: 1.6, color: "#94A3B8", whiteSpace: "pre-wrap" }}>{m.body}</div></div>{i < list.length - 1 && <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 16px" }}><div style={{ flex: 1, height: 1, background: "#1E293B" }} /><span style={{ color: "#475569", fontSize: 9 }}>{list[i + 1]?.delay_days === 0 ? "Immediately" : `Wait ${list[i + 1]?.delay_days}d`}</span><div style={{ flex: 1, height: 1, background: "#1E293B" }} /></div>}</div>))}</div></div>); };
...
function ContactModal({ c, team, user, onClose, onSave }) { const [f, setF] = useState({ name: c?.name || "", ig: c?.ig || "", email: c?.email || "", youtube: c?.youtube || "", website: c?.website || "", notes: c?.notes || "", pipeline_value: c?.pipeline_value || "", assigned_to: c?.assigned_to || user }); const ref = useRef(null); useEffect(() => { ref.current?.focus(); }, []); const save = () => { if (!f.name.trim()) return; onSave({ ...f, pipeline_value: parseFloat(f.pipeline_value) || 0 }); }; return (<div style={S.ov} onClick={onClose}><div style={S.modal} onClick={e => e.stopPropagation()}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><h2 style={{ color: "#F1F5F9", fontSize: 17, fontWeight: 700, fontFamily: "'Outfit',sans-serif", margin: 0 }}>{c ? "Edit Lead" : "Add New Lead"}</h2><button style={S.x} onClick={onClose}>✕</button></div><div style={S.fg2}><div style={S.fi}><label style={S.lb}>Name *</label><input ref={ref} style={S.ip} value={f.name} onChange={e => setF({ ...f, name: e.target.value })} placeholder="John Smith" onKeyDown={e => e.key === "Enter" && save()} /></div><div style={S.fi}><label style={S.lb}>Assigned To</label><select style={S.ip} value={f.assigned_to} onChange={e => setF({ ...f, assigned_to: e.target.value })}>{team.map(t => <option key={t} value={t}>{t}</option>)}</select></div><div style={S.fi}><label style={S.lb}>Instagram</label><input style={S.ip} value={f.ig} onChange={e => setF({ ...f, ig: e.target.value })} placeholder="@handle" /></div><div style={S.fi}><label style={S.lb}>Email</label><input style={S.ip} value={f.email} onChange={e => setF({ ...f, email: e.target.value })} placeholder="john@email.com" /></div><div style={S.fi}><label style={S.lb}>YouTube</label><input style={S.ip} value={f.youtube} onChange={e => setF({ ...f, youtube: e.target.value })} placeholder="Channel URL" /></div><div style={S.fi}><label style={S.lb}>Website</label><input style={S.ip} value={f.website} onChange={e => setF({ ...f, website: e.target.value })} placeholder="https://..." /></div><div style={S.fi}><label style={S.lb}>Pipeline Value ($)</label><input style={S.ip} type="number" value={f.pipeline_value} onChange={e => setF({ ...f, pipeline_value: e.target.value })} placeholder="5000" /></div><div style={{ ...S.fi, gridColumn: "1/-1" }}><label style={S.lb}>Notes</label><textarea style={{ ...S.ip, minHeight: 60, resize: "vertical" }} value={f.notes} onChange={e => setF({ ...f, notes: e.target.value })} placeholder="Notes..." /></div></div><div style={{ display: "flex", gap: 6, justifyContent: "flex-end", marginTop: 16 }}><button style={S.ghost} onClick={onClose}>Cancel</button><button style={{ ...S.pri, opacity: f.name.trim() ? 1 : .5 }} onClick={save} disabled={!f.name.trim()}>{c ? "Save" : "Add Lead"}</button></div></div></div>); }

function MsgModal({ m, total, type, onClose, onSave }) { const [f, setF] = useState({ name: m?.name || `${type === "nurture" ? "Nurture" : "Message"} ${total + 1}`, channel: m?.channel || "ig", delay_days: m?.delay_days ?? 3, body: m?.body || "" }); return (<div style={S.ov} onClick={onClose}><div style={S.modal} onClick={e => e.stopPropagation()}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><h2 style={{ color: "#F1F5F9", fontSize: 17, fontWeight: 700, fontFamily: "'Outfit',sans-serif", margin: 0 }}>{m ? "Edit" : "Add"} Message</h2><button style={S.x} onClick={onClose}>✕</button></div><div style={S.fg2}><div style={S.fi}><label style={S.lb}>Name</label><input style={S.ip} value={f.name} onChange={e => setF({ ...f, name: e.target.value })} /></div><div style={S.fi}><label style={S.lb}>Channel</label><select style={S.ip} value={f.channel} onChange={e => setF({ ...f, channel: e.target.value })}><option value="ig">Instagram DM</option><option value="email">Email</option></select></div><div style={S.fi}><label style={S.lb}>Delay (days)</label><input style={S.ip} type="number" min="0" value={f.delay_days} onChange={e => setF({ ...f, delay_days: parseInt(e.target.value) || 0 })} /></div><div style={{ ...S.fi, gridColumn: "1/-1" }}><label style={S.lb}>Body</label><textarea style={{ ...S.ip, minHeight: 120, resize: "vertical" }} value={f.body} onChange={e => setF({ ...f, body: e.target.value })} placeholder={'Use {{name}} for auto-fill'} /><p style={{ color: "#475569", fontSize: 10, marginTop: 2 }}>{'{{name}}'} auto-fills with lead&apos;s first name</p></div></div><div style={{ display: "flex", gap: 6, justifyContent: "flex-end", marginTop: 16 }}><button style={S.ghost} onClick={onClose}>Cancel</button><button style={S.pri} onClick={() => onSave(f)}>{m ? "Save" : "Add"}</button></div></div></div>); }
