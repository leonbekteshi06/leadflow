...
export default function CRM() {
  ...
  const getNextN = (c) => { const seq = c.nurture_sequence || "Default"; let seqMsgs = nurtureM.filter(m => (m.sequence_name || "Default") === seq).sort((a, b) => a.step - b.step); if (seqMsgs.length === 0) seqMsgs = nurtureM.filter(m => (m.sequence_name || "Default") === "Default").sort((a, b) => a.step - b.step); if (seqMsgs.length === 0) seqMsgs = nurtureM.sort((a, b) => a.step - b.step); const idx = c.nurture_step || 0; if (idx < seqMsgs.length) { const m = seqMsgs[idx]; return { ...m, body: m.body.replace(/\{\{name\}\}/g, c.name.split(" ")[0]) }; } if (seqMsgs.length > 0) { const m = seqMsgs[idx % seqMsgs.length]; return { ...m, body: m.body.replace(/\{\{name\}\}/g, c.name.split(" ")[0]) }; } return null; };
