  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.main}>
        ...
        {view === "dashboard" && (<div style={S.content}><div style={S.header}><div><h1 style={S.h1}>Dashboard</h1><p style={S.sub}>Welcome back, {user}</p></div><button style={S.pri} onClick={() => setModal({ type: "contact", data: null })}>+ Add Lead</button></div><StandupFeed /><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(145px,1fr))", gap: 8, marginBottom: 20 }}>{[{ l: "Contacted This Week", v: stats.contactedWeek, c: "#3B82F6" }, { l: "Contacted This Month", v: stats.contactedMonth, c: "#6366F1" }, { l: "Pipeline Value", v: fmtMoney(stats.pipeline), c: "#10B981" }, { l: "Closed This Month", v: fmtMoney(stats.closedMonth), c: "#F59E0B" }, { l: "Total Revenue", v: fmtMoney(stats.closedTotal), c: "#10B981" }, { l: "Close Rate", v: `${stats.convRate}%`, c: "#8B5CF6" }].map((s, i) => (<div key={i} style={{ padding: 12, background: "#0F172A", borderRadius: 10, border: "1px solid #1E293B", borderTop: `3px solid ${s.c}` }}><div style={{ color: "#64748B", fontSize: 10, fontWeight: 600, textTransform: "uppercase" }}>{s.l}</div><div style={{ color: "#F1F5F9", fontSize: 20, fontWeight: 700, fontFamily: "'Outfit',sans-serif", marginTop: 3 }}>{s.v}</div></div>))}</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 6, marginBottom: 20 }}>{STAGES.map(s => { const cnt = contacts.filter(c => c.stage === s.id).length; return (<div key={s.id} style={{ padding: "12px 10px", background: "#0F172A", borderRadius: 8, border: "1px solid #1E293B", borderLeft: `3px solid ${s.color}`, cursor: "pointer" }} onClick={() => { setView("contacts"); setFilter(s.id); }}><div style={{ color: "#94A3B8", fontSize: 10, fontWeight: 600 }}>{s.label}</div><div style={{ color: "#F1F5F9", fontSize: 22, fontWeight: 700, fontFamily: "'Outfit',sans-serif" }}>{cnt}</div></div>); })}</div><h2 style={S.h2}>Today&apos;s Non-Negotiables</h2>{(dailyCats.length + weeklyCats.length) === 0 ? <div style={S.empty}><p style={{ color: "#64748B" }}>No non-negotiables set yet. Add them under KPIs → Edit KPIs.</p></div> : <div style={{ marginBottom: 20 }}><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 8, marginBottom: 10 }}>{dailyCats.map(cat => { const cnt = getDayCount(user, cat.name, todayStr()); const t = cat.daily_target || 0; const hit = t > 0 && cnt >= t; const col = catColor(cat.name); return (<div key={cat.id} style={{ padding: 10, background: "#0F172A", borderRadius: 10, border: `1px solid ${hit ? "#10B98140" : "#1E293B"}`, borderLeft: `3px solid ${col}` }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><span style={{ color: col, fontSize: 12, fontWeight: 600 }}>{cat.name}</span>{hit && <span style={{ color: "#10B981", fontSize: 12 }}>✓</span>}</div><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}><button onClick={() => logKpi(user, cat.name, -1)} style={{ ...S.kpiBtn, opacity: cnt > 0 ? 1 : 0.3 }}>-</button><span style={{ fontSize: 18, fontWeight: 700, color: "#F1F5F9", fontFamily: "'Outfit',sans-serif" }}>{cnt}{t > 0 && <span style={{ color: "#475569", fontSize: 11 }}>/{t}</span>}</span><button onClick={() => logKpi(user, cat.name, 1)} style={{ ...S.kpiBtn, background: col + "20", borderColor: col + "40", color: col }}>+</button></div></div>); })}{weeklyCats.map(cat => { const cnt = getWeeklyCount(user, cat.name); const t = cat.weekly_target || 0; const hit = t > 0 && cnt >= t; const col = catColor(cat.name); return (<div key={cat.id} style={{ padding: 10, background: "#0F172A", borderRadius: 10, border: `1px solid ${hit ? "#10B98140" : "#1E293B"}`, borderLeft: `3px solid ${col}` }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><span style={{ color: col, fontSize: 12, fontWeight: 600 }}>{cat.name} <span style={{ color: "#64748B", fontSize: 9 }}>/wk</span></span>{hit && <span style={{ color: "#10B981", fontSize: 12 }}>✓</span>}</div><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}><button onClick={() => logKpi(user, cat.name, -1)} style={{ ...S.kpiBtn, opacity: cnt > 0 ? 1 : 0.3 }}>-</button><span style={{ fontSize: 18, fontWeight: 700, color: "#F1F5F9", fontFamily: "'Outfit',sans-serif" }}>{cnt}{t > 0 && <span style={{ color: "#475569", fontSize: 11 }}>/{t}</span>}</span><button onClick={() => logKpi(user, cat.name, 1)} style={{ ...S.kpiBtn, background: col + "20", borderColor: col + "40", color: col }}>+</button></div></div>); })}</div>{dailyCats.length > 0 && TEAM.length > 1 && <div style={{ background: "#0F172A", borderRadius: 10, border: "1px solid #1E293B", padding: "10px 12px" }}><div style={{ fontSize: 10, color: "#64748B", fontWeight: 700, textTransform: "uppercase", marginBottom: 8 }}>Who&apos;s on track today</div><div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{TEAM.map(person => { const targets = dailyCats.filter(cat => (cat.daily_target || 0) > 0); const hitCount = targets.filter(cat => getDayCount(person, cat.name, todayStr()) >= cat.daily_target).length; const total = targets.length; const allHit = total > 0 && hitCount === total; return (<div key={person} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}><span style={{ color: "#F1F5F9", fontSize: 12, fontWeight: 600 }}>{person}{person === user && <span style={{ color: "#3B82F6", fontSize: 9 }}> (you)</span>}</span><div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ display: "flex", gap: 3 }}>{dailyCats.map(cat => { const t = cat.daily_target || 0; const hit = t > 0 && getDayCount(person, cat.name, todayStr()) >= t; return <span key={cat.id} title={cat.name} style={{ width: 9, height: 9, borderRadius: "50%", background: hit ? catColor(cat.name) : "#1E293B", border: `1px solid ${hit ? catColor(cat.name) : "#334155"}` }} />; })}</div><span style={{ fontSize: 11, fontWeight: 600, color: allHit ? "#10B981" : "#94A3B8" }}>{hitCount}/{total}</span></div></div>); })}</div></div>}</div>}<h2 style={S.h2}>Team</h2><div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>{TEAM.map(t => { const ml = contacts.filter(c => c.assigned_to === t); const ma = actionsDue.filter(c => c.assigned_to === t); const mc = ml.filter(c => c.stage === "closed").reduce((s, c) => s + (c.closed_value || 0), 0); return (<div key={t} style={{ padding: 12, background: "#0F172A", borderRadius: 10, border: `1px solid ${t === user ? "#3B82F640" : "#1E293B"}` }}><div style={{ color: "#F1F5F9", fontWeight: 600, fontSize: 14, marginBottom: 6 }}>{t}{t === user && <span style={{ color: "#3B82F6", fontSize: 10 }}> (you)</span>}</div><div style={{ display: "flex", flexDirection: "column", gap: 3 }}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span style={{ color: "#64748B" }}>Leads</span><span style={{ color: "#CBD5E1", fontWeight: 600 }}>{ml.length}</span></div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span style={{ color: "#64748B" }}>Due Today</span><span style={{ color: ma.length ? "#F59E0B" : "#CBD5E1", fontWeight: 600 }}>{ma.length}</span></div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span style={{ color: "#64748B" }}>Closed</span><span style={{ color: "#10B981", fontWeight: 600 }}>{fmtMoney(mc)}</span></div></div></div>); })}</div></div>)}
