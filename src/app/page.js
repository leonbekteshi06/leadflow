...
export default function CRM() {
  ...
  const markSent = async (id) => { const c = contacts.find(x => x.id === id); if (!c) return; const ns = c.current_step + 1; const nm = messages.filter(m => (m.sequence_name || "Default") === (c.outreach_sequence || "Default")).find(m => m.step === ns + 1); const cm = messages.filter(m => (m.sequence_name || "Default") === (c.outreach_sequence || "Default")).find(m => m.step === ns); await updateContact(id, { current_step: ns, stage: ns > 0 && c.stage === "new" ? "outreach" : c.stage, last_contacted_at: todayStr(), next_follow_up: nm ? addDays(todayStr(), nm.delay_days) : null, history: [...(c.history || []), { step: ns, name: cm?.name || `Msg ${ns}`, at: todayStr() }] }); if (c.current_step === 0) { await logKpi(user, "DMs", 1); } logActivity(user, "sent_outreach", `${cm?.name} to ${c.name}`); flash("Marked sent!"); };
  ...
  const getNext = (c) => { const seq = c.outreach_sequence || "Default"; const seqMsgs = messages.filter(m => (m.sequence_name || "Default") === seq); const m = seqMsgs.find(x => x.step === c.current_step + 1); return m ? { ...m, body: m.body.replace(/\{\{name\}\}/g, c.name.split(" ")[0]) } : null; };
  const getNextN = (c) => { const seq = c.nurture_sequence || "Default"; const seqMsgs = nurtureM.filter(m => (m.sequence_name || "Default") === seq); let m = seqMsgs.find(x => x.step === (c.nurture_step || 0) + 1); if (!m && seqMsgs.length > 0) { const weekly = seqMsgs.filter(x => x.step >= 7); if (weekly.length > 0) m = weekly[((c.nurture_step || 0) - 6) % weekly.length] || weekly[0]; else m = seqMsgs[seqMsgs.length - 1]; } return m ? { ...m, body: m.body.replace(/\{\{name\}\}/g, c.name.split(" ")[0]) } : null; };
  ...
  const MsgView = ({ type }) => {
    const [activeSeq, setActiveSeq] = useState("Default");
    ...
    const list = allList.filter(m => (m.sequence_name || "Default") === activeSeq);
    ...
    return (<div style={S.content}>
      <div style={S.header}><div><h1 style={S.h1}>{type === "outreach" ? "Outreach Sequences" : "Nurture Sequences"}</h1><p style={S.sub}>Manage multiple sequences for different ICPs</p></div><div style={{ display: "flex", gap: 6 }}><button style={S.ghost} onClick={() => setShowNewSeq(!showNewSeq)}>+ New Sequence</button><button style={S.pri} onClick={() => setModal({ type: "msg", data: null, msgType: type, seqName: activeSeq })}>+ Add Message</button></div></div>
